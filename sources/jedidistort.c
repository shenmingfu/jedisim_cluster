...
#define OMEGA_M 0.308       //matter density of the universe today
#define OMEGA_D 0.692       //dark energy density of the universe
#define OPZEQ   3366        //1 + Z_eq
#define C_H_0   4421.718    //c [km/s] / Hubble constant today [km/(s*Mpc)] with H_0 = 67.80
#define G       4.301       //Gravitational constant [10^{-3} (pc/Solar mass) (km/s)^2]
...
void get_alpha(long int x, long int y, long int nlenses, lens* lenses, float* alphax, float* alphay);
...
int main(int argc, char *argv[]){
	...
	for(nlens = 0; nlens < nlenses; nlens++){
		...
		if(lenses[nlens].table == NULL){
			fprintf(stderr,"Error allocating memory for lens %li table.\n", nlens);
		}
			...
			case 2: //Navarro-Frenk-White profile
				{
					...
					for(a = 1; a < lenses[nlens].nr; a++){
						...
						if(xvar > 0 && xvar < 1)
							...
							fprintf(stdout,"Error: could not calculate lens %li profile. xvar outside of range (0, infty). xvar: %f", nlens, xvar);
							exit(1);
						}
		...
		fprintf(stdout,"Calculated profile for lens %li.\n\n", nlens);
	}
	fclose(lensfile);
	...
	for(gal = 0; gal < ngalaxies; gal++){
		...
		if(ixmin > ixmax || iymin > iymax){
			fprintf(stdout,"Warning: galaxy %li is completely of the screen.\n", gal);
			ixmin = 0;
			ixmax = 1;
			iymin = 0;
			iymax = 1;
	...
	for(gr = 0; gr< ngr; gr++){
        ...
        for(ngal = 0; ngal < ngalaxies; ngal++){
            free(galaxies[ngal].file); 
            free(galaxies[ngal].outfile);
        }
        free(galaxies);
        for(nlens = 0; nlens < nlenses; nlens++){
            free(lenses[nlens].table);
        }
        free(lenses);
	return 0;
}
