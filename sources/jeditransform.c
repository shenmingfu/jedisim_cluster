...
#define PI  3.14159265         //because C doesn't have it built-in for some silly reason
#define EPSILON 0.0000001   //a very small number
#define MAG0 26.4912           //magnitude zero point
...
float bilinear_interp(float x, float y, long int xmax, long int ymax, float *img);
...
int main(int argc, char *argv[]){
    //declare variables
    long int     ngalaxies;          //the number of galaxies  
    ...
    fprintf(stdout,"Found %li galaxies in galaxy list \"%s\"\n", ngalaxies, gallist_path);
    ...
    for(g = 0; g < ngalaxies; g++){
        ...
        if(galaxies[g].image == NULL || galaxies[g].stamp1 == NULL || galaxies[g].stamp2 == NULL){
            fprintf(stderr,"Error: could not allocate memory for galaxy  %li.\n", g);
            exit(1);
        }
    ...
    fclose(gallist_file);
    fprintf(stdout,"Read in %li galaxies.\n", ngalaxies);
    ...
    for(g = 0; g < ngalaxies; g++){
        ...
        if (galaxy == NULL){
            fprintf(stderr, "Error: cannot allocate memory for galaxy %li image.\n", g);
            exit(1);
        }
        ...
        if(fits_read_pix(galfptr, TFLOAT, fpixel, galnaxes[0]*galnaxes[1], NULL, galaxy, NULL, &status)){
            fprintf(stderr, "Error: cannot read in galaxy %li image.\n", g);
            fits_report_error(stderr, status);
            exit(1);
        }
        ...
        fits_report_error(stderr, status);
        fprintf(stdout, "Transformed image %li.\n", g);
    ...
    for(g = 0; g < ngalaxies; g++){
        ...
        free(galaxies[g].stamp1);
        free(galaxies[g].stamp2);
    }
    free(galaxies);    
...
float bilinear_interp(float x, float y, long int xmax, long int ymax, float *img){
    long int xi = (long int) x, yi = (long int) y;
